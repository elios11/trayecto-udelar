...
test("integra una sola proyección por identidad canónica vigente y excluye planes históricos verificados", () => {
  ...
  assert.equal(report.counts.generatedPlans, 145);
  assert.equal(report.counts.excludedFromCurrentUi, 32);
  ...
  assert.equal(catalog.flatMap((faculty) => faculty.careers).flatMap((career) => career.plans).length, 145);
