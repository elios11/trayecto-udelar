...
test("integra una sola proyección por identidad canónica vigente y excluye planes históricos verificados", () => {
  ...
  assert.equal(report.counts.generatedPlans, 174);
  assert.equal(report.counts.excludedFromCurrentUi, 3);
  ...
  assert.equal(catalog.flatMap((faculty) => faculty.careers).flatMap((career) => career.plans).length, 174);
