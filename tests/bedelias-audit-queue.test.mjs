...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 128);
  assert.equal(saved.counts.pendingCanonicalIdentities, 49);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 49,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en astronomia:2016");
});
