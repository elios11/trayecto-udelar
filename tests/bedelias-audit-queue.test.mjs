...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 31);
  assert.equal(saved.counts.pendingCanonicalIdentities, 146);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 37,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en diseno integrado:2012");
});
