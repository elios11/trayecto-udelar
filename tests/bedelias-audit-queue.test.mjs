...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 104);
  assert.equal(saved.counts.pendingCanonicalIdentities, 73);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 73,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnicatura en artes plasticas y visuales:2017");
});
