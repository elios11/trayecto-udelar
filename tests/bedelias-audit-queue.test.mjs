...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 143);
  assert.equal(saved.counts.pendingCanonicalIdentities, 34);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 34,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "antropologia:2014");
});
