...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 160);
  assert.equal(saved.counts.pendingCanonicalIdentities, 17);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 17,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria fisico matematica:2017");
});
