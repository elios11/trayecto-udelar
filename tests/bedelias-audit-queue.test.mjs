...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 35);
  assert.equal(saved.counts.pendingCanonicalIdentities, 142);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 33,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria forestal:2013");
});
