...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 113);
  assert.equal(saved.counts.pendingCanonicalIdentities, 64);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 64,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en artes artes plasticas y visuales:2002");
});
