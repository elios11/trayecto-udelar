...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 156);
  assert.equal(saved.counts.pendingCanonicalIdentities, 21);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 21,
  });
  ...
  assert.equal(saved.queue.filter((entry) => entry.sourceOffers.length > 1).length, 2);
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria de alimentos:2003");
});
