...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 36);
  assert.equal(saved.counts.pendingCanonicalIdentities, 141);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 32,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnicatura en desarrollo regional sustentable:2013");
});
