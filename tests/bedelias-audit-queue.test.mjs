...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 138);
  assert.equal(saved.counts.pendingCanonicalIdentities, 39);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 39,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "traductorado publico aleman:2022");
});
