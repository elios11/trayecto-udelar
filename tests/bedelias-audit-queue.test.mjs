...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 45);
  assert.equal(saved.counts.pendingCanonicalIdentities, 132);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 23,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en desarrollo:2009");
});
