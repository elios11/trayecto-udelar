...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 144);
  assert.equal(saved.counts.pendingCanonicalIdentities, 33);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 33,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "correccion de estilo:2014");
});
