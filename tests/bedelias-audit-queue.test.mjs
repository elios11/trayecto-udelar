...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 161);
  assert.equal(saved.counts.pendingCanonicalIdentities, 16);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 16,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria industrial mecanica:1997");
});
