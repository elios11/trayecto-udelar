...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 162);
  assert.equal(saved.counts.pendingCanonicalIdentities, 15);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 15,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria naval:1997");
});
