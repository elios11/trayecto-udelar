...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 15);
  assert.equal(saved.counts.pendingCanonicalIdentities, 162);
  assert.deepEqual(saved.counts.byPriority, {
    "insufficient-regional-content": 15,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "lic en ingenieria biologica:2013");
});
