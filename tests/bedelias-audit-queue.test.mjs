...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 175);
  assert.equal(saved.counts.pendingCanonicalIdentities, 2);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 2,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnicatura en guardavidas:2025");
});
