...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 166);
  assert.equal(saved.counts.pendingCanonicalIdentities, 11);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 11,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnologo en informatica:2007");
});
