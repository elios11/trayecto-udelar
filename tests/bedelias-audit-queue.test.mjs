...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 119);
  assert.equal(saved.counts.pendingCanonicalIdentities, 58);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 58,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en danza contemporanea:2018");
});
