...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 150);
  assert.equal(saved.counts.pendingCanonicalIdentities, 27);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 27,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "letras:2014");
});
