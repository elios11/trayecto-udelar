...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 148);
  assert.equal(saved.counts.pendingCanonicalIdentities, 29);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 29,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "historia:2014");
});
