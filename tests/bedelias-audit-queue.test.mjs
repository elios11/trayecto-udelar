...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 154);
  assert.equal(saved.counts.pendingCanonicalIdentities, 23);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 23,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "museologia:2011");
});
