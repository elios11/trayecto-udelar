...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 169);
  assert.equal(saved.counts.pendingCanonicalIdentities, 8);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 8,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "bachiller en ciencias quimicas:2000");
});
