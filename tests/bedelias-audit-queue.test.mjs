...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 133);
  assert.equal(saved.counts.pendingCanonicalIdentities, 44);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 44,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en fisica medica:2025");
});
