...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 149);
  assert.equal(saved.counts.pendingCanonicalIdentities, 28);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 28,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "interpretacion lsu espanol lsu:2014");
});
