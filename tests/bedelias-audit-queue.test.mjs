...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 172);
  assert.equal(saved.counts.pendingCanonicalIdentities, 5);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 5,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en tecnologias de la quimica:2022");
});
