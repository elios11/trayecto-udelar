...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 116);
  assert.equal(saved.counts.pendingCanonicalIdentities, 61);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 61,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en artes diseno grafico:2002");
});
