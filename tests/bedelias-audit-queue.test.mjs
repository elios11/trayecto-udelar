...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 101);
  assert.equal(saved.counts.pendingCanonicalIdentities, 76);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 76,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnologo en sistemas integrados de produccion agropecuaria:2022");
});
