...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 158);
  assert.equal(saved.counts.pendingCanonicalIdentities, 19);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 19,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria en agrimensura:2023");
});
