...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 147);
  assert.equal(saved.counts.pendingCanonicalIdentities, 30);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 30,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "filosofia:2010");
});
