...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 145);
  assert.equal(saved.counts.pendingCanonicalIdentities, 32);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 32,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "dramaturgia:2015");
});
