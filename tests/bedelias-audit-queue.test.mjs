...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 18);
  assert.equal(saved.counts.pendingCanonicalIdentities, 159);
  assert.deepEqual(saved.counts.byPriority, {
    "insufficient-regional-content": 12,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en instrumentacion quirurgica:2006");
});
