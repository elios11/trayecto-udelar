...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 41);
  assert.equal(saved.counts.pendingCanonicalIdentities, 136);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 27,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnico rural:1956");
});
