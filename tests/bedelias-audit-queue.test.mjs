...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 168);
  assert.equal(saved.counts.pendingCanonicalIdentities, 9);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 9,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnologo industrial mecanico:2016");
});
