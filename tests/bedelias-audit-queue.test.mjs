...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 146);
  assert.equal(saved.counts.pendingCanonicalIdentities, 31);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 31,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "educacion:2014");
});
