...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 115);
  assert.equal(saved.counts.pendingCanonicalIdentities, 62);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 62,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en artes dibujo y pintura:2002");
});
