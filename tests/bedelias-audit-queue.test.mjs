...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 159);
  assert.equal(saved.counts.pendingCanonicalIdentities, 18);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 18,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria en sistemas de comunicacion:2018");
});
