...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 32);
  assert.equal(saved.counts.pendingCanonicalIdentities, 145);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 36,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnologo en administracion y contabilidad:2012");
});
