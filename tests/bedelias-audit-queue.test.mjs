...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 121);
  assert.equal(saved.counts.pendingCanonicalIdentities, 56);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 56,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en musica:2005");
});
