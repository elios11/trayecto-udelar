...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 137);
  assert.equal(saved.counts.pendingCanonicalIdentities, 40);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 40,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en relaciones internacionales:2013");
});
