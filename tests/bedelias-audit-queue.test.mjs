...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 170);
  assert.equal(saved.counts.pendingCanonicalIdentities, 7);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 7,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "bioquimico clinico:2015");
});
