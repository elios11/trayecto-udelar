...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 47);
  assert.equal(saved.counts.pendingCanonicalIdentities, 130);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 21,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "diplomacia:1918");
});
