...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 134);
  assert.equal(saved.counts.pendingCanonicalIdentities, 43);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 43,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en geografia:2018");
});
