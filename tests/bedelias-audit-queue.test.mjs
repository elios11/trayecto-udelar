...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 19);
  assert.equal(saved.counts.pendingCanonicalIdentities, 158);
  assert.deepEqual(saved.counts.byPriority, {
    "insufficient-regional-content": 11,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en laboratorio clinico:2006");
});
