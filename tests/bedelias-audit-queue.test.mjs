...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 54);
  assert.equal(saved.counts.pendingCanonicalIdentities, 123);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 16,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnicatura en electroencefalografia:1900");
});
