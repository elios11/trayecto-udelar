...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 108);
  assert.equal(saved.counts.pendingCanonicalIdentities, 69);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 69,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en diseno de paisaje:2008");
});
