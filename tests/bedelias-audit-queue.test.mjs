...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 112);
  assert.equal(saved.counts.pendingCanonicalIdentities, 65);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 65,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en arte digital y electronico:2013");
});
