...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 157);
  assert.equal(saved.counts.pendingCanonicalIdentities, 20);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 20,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "ingenieria de produccion:2010");
});
