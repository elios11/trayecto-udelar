...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 107);
  assert.equal(saved.counts.pendingCanonicalIdentities, 70);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 70,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnologo en madera:2012");
});
