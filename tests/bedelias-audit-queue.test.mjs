...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 111);
  assert.equal(saved.counts.pendingCanonicalIdentities, 66);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 66,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en economia agricola y gestion de agronegocios:2022");
});
