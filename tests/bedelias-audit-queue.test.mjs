...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 155);
  assert.equal(saved.counts.pendingCanonicalIdentities, 22);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 22,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en ingenieria de medios:2018");
});
