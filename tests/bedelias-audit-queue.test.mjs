...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 48);
  assert.equal(saved.counts.pendingCanonicalIdentities, 129);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 20,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en relaciones laborales:2012");
});
