...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 46);
  assert.equal(saved.counts.pendingCanonicalIdentities, 131);
  assert.deepEqual(saved.counts.byPriority, {
    "composition-unavailable": 22,
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "licenciatura en sociologia:2009");
});
