...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 173);
  assert.equal(saved.counts.pendingCanonicalIdentities, 4);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 4,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "quimico:2015");
});
