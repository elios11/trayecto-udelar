...
test("genera una cola canónica deduplicada y dirigida por excepciones", () => {
  ...
  assert.equal(saved.counts.evidenceClosedCanonicalIdentities, 106);
  assert.equal(saved.counts.pendingCanonicalIdentities, 71);
  assert.deepEqual(saved.counts.byPriority, {
    "official-sources-pending": 71,
  });
...
test("la cola guardada se reconstruye sin red con el mismo hash", () => {
  ...
  assert.equal(rebuilt.queue[0].identity, "tecnicatura en gestion de recursos naturales:2011");
});
