...
const audit = registry.audits.find((entry) => entry.identity === "abogacia:2016");
const canonical = readJson("data/bedelias/fder-abogacia-2016.json");
const regional = readJson("data/bedelias/cenurln-abogacia-2016.json");
...
test("la auditoría oficial conserva los 450 créditos y los mínimos por área", () => {
  assert.equal(audit.status, "official-evidence-complete");
  assert.equal(audit.publicationEligible, false);
  assert.equal(audit.officialPlan.title, "Abogado");
  assert.equal(audit.officialPlan.durationMonths, 60);
  assert.equal(audit.officialPlan.minimumCredits, 450);
  assert.equal(Object.values(audit.officialPlan.minimumCreditsByArea)
    .reduce((total, credits) => total + credits, 0), 450);
  assert.equal(audit.officialPlan.intermediateTitle.title, "Procurador");
  assert.equal(audit.officialPlan.intermediateTitle.minimumCredits, 160);
});

test("Salto conserva el núcleo y amplía únicamente la bolsa flexible", () => {
  ...
  assert.equal(changedCredits.length, 1);
