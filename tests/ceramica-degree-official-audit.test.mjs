...
const plan = await readJson("app/data/bedelias-generated/bedelias-fartes-licenciatura-en-artes-ceramica-2002.json");

test("normaliza la Licenciatura en Artes Cerámica a seis años y 330 créditos", () => {
  const audit = registry.audits.find((entry) => entry.identity === "licenciatura en artes ceramica:2002");
  ...
  assert.equal(plan.plan.degreeTitle, "Licenciado en Artes - Cerámica");
  assert.deepEqual(plan.campuses.map((campus) => campus.label), ["Montevideo"]);
});

test("proyecta diez bloques oficiales sin sumar cátedras alternativas", () => {
...
test("conserva Artes del Fuego dentro del taller y exige el Trabajo Final", () => {
  const workshopCredits = plan.courses
    .filter((course) => /actividades en Artes del Fuego/.test(course.name))
