...
`npm run bedelias:project` genera `app/data/computacion-1997-electivas.json` con todas las unidades curriculares de la composición del Plan 1997 que no forman parte de la carga inicial. Bedelías determina inclusión, código, créditos y áreas posibles; sólo se omiten entradas administrativas de créditos por reválida o no acumulables. La interfaz importa este archivo cuando la persona solicita el catálogo completo. `data/fing/computacion-oferta-2026-2.json` es opcional y sólo agrega semestre, cupo y enlace cuando existe: una materia de Bedelías nunca desaparece por faltar en EVA, horarios u otra publicación.
